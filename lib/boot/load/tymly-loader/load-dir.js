...
module.exports = function loadDir (rootDir, allComponents, options) {
  ...
  for (const { dir, name } of componentTypesToLoad) {
    if (!rootComponents.hasOwnProperty(name)) {
      rootComponents[name] = {}
    }
    ...
    for (const filename of dirContent) {
      ...
      if (stats.isFile()) {
        ...
        loaded = loader(parsedMetaJson, key, path.join(dir, filename), options.messages)
      } else {
        loaded = loadComponentDir(parsedMetaJson, key, path.join(dir, filename), options.messages)
