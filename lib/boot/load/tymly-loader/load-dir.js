...
module.exports = function loadDir (
  ...
  for (const { dir, name } of findComponentTypesToLoad(rootDir)) {
    if (name === 'changelog' || name === 'blueprintJson' || name === 'pluginJson') {
      ...
      for (const filename of dirContent) {
        ...
        if (loaded.refs) {
          if (!tymlyRefs.hasOwnProperty(name)) tymlyRefs[name] = {}
...
function findComponentTypesToLoad (rootDir) {
  const componentDirectories = fs.readdirSync(rootDir)
    .filter(dir => fs.statSync(path.join(rootDir, dir)).isDirectory() || dir === 'CHANGELOG.md' || dir === 'blueprint.json')
    .filter(dir => dir[0] !== '.')
  ...
  const componentTypesToLoad = componentDirectories
    .map(dir => {
      ...
      if (dir === 'blueprint.json') name = 'blueprintJson'

      return { dir: path.join(rootDir, dir), name }
    })
    .filter(component => !COMPONENT_TYPE_BLACKLIST.includes(component.name))

  if (rootDir.endsWith('/components')) {
    const dir = path.join(rootDir, '..', '..', 'plugin.json')

    if (fs.existsSync(dir)) {
      componentTypesToLoad.push({ dir, name: 'pluginJson' })
    }
  }
