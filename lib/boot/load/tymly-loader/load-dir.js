...
module.exports = function loadDir (rootDir, allComponents, options) {
  ...
  for (const { dir, name } of componentTypesToLoad) {
    ...
    for (const filename of dirContent) {
      ...
      const loadComponent = stats.isFile() ? loadComponentFile : loadComponentDir
      const loaded = loadComponent(parsedMetaJson, key, filepath, options.messages, includeDocumentation)
