...
module.exports = function bootSequenceOrder (serviceComponents, messages) {
  ...
  let depthLimit = Object.keys(serviceComponents).length
  ...
  addPriorServices(null, Object.keys(serviceComponents), 0)

  if (hasErrors) return

  const orderedServiceComponents = []
  orderedServiceNames.reverse()
...
function applyBootBefore (serviceComponents, messages) {
  ...
  for (const [ serviceName, serviceComponent ] of Object.entries(serviceComponents)) {
