...
class WebsocketServerService {
  boot (options) {
    ...
    this.webSocketServer.on('connection', (ws, req) => {
      ws.on('error', err => { debug('connection error', err) }) // ignore if ECONNRESET
      ws.on('close', code => { debug('connection close', code) })

      this.clients = this.clients.filter(({ ws }) => ![2, 3].includes(ws.readyState))
      this.clients.push({ ws })

      debug(`connection (${this.clients.length} connection${this.clients.length === 1 ? '' : 's'})`)
