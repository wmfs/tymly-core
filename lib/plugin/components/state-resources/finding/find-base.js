...
function resolveInputPaths (input, template) {
  ...
  const { where } = clonedTemplate

  const removeNulls = (property, conditions) => {
    for (const [k, v] of Object.entries(conditions)) {
      if (k !== 'includeNulls' && (v === '' || v === null || v === undefined)) {
        delete where[property][k]
      }
    }
    delete where[property].includeNulls
  }

  if (where) {
    for (const [property, conditions] of Object.entries(where)) {
      if (!conditions.includeNulls) removeNulls(property, conditions)
      if (!Object.keys(where[property]).length) delete where[property]
    }
  }
